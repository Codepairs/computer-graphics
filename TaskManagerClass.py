...
class TaskManager:
    ...
    @staticmethod
    def task19(matrix_size: tuple, color: list[int], model_num: int, rotate_x, rotate_y, rotate_z):
        model = TaskManager.choose_model_new(model_num, matrix_size)
        image = np.zeros(matrix_size + (3,), dtype=np.uint8)
        ...
        Renderer.draw_model_phong_shading(image, color, model, z_buffer, rotate_x, rotate_y, rotate_z)
